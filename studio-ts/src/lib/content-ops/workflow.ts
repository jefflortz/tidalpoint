import {adaptRankScoreArticle, fetchRankScoreArticles} from './adapters'
...
export async function discoverWeeklyArticle(now = new Date()) {
  ...
  const preliminary = adaptRankScoreArticle(candidate, pillars[0]?._id ?? '')
  ...
  const article = adaptRankScoreArticle(candidate, choice.pillarId)
